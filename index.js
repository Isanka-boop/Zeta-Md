...
const pairingExpireTimers = {}; // sessionId -> timeout that kills the session if the pairing code isn't used in time

const PAIRING_CODE_TTL_MS = 3 * 60 * 1000; // 3 minutes
...
function cleanupSession(sessionId) {
    ...
    if (pairingExpireTimers[sessionId]) { clearTimeout(pairingExpireTimers[sessionId]); delete pairingExpireTimers[sessionId]; }
    ...
    try {
        ...
        if (!sock.authState.creds.registered) {
            try {
                ...
                if (res && !res.headersSent) { res.json({ code: pairingCode, expiresAt, expiresInSec: PAIRING_CODE_TTL_MS / 1000 }); responded = true; }

                // ── Pairing code expiry: if the number isn't linked within 3 minutes, kill the session ──
                pairingExpireTimers[sessionId] = setTimeout(async () => {
                    delete pairingExpireTimers[sessionId];
                    if (!sock.authState?.creds?.registered) {
                        console.log(`⏰ Pairing code expired (unused): ${sessionId}`);
                        cleanupSession(sessionId);
                        try {
                            await Session.findOneAndDelete({ sessionId });
                            await fs.remove(sessionPath);
                        } catch (expireErr) {
                            console.error('[PAIRING EXPIRE CLEANUP ERROR]', expireErr);
                        }
                    }
                }, PAIRING_CODE_TTL_MS);
            } catch (pairErr) {
...
> © ${config.BOT_NAME || 'Zeta'} • All rights reserved.`;
                    ...
                    try {
                        ...
                        const fallbackUrl = 'https://files.catbox.moe/chtymz.jpg';
...
// ── Exports for plugins (e.g. plugins/pair.js, plugins/delsession.js) ─────────
// If this file isn't named index.js in your project, update the require path
// used inside those plugins (e.g. require('../index') -> require('../<this-filename>')).
module.exports = {
    Pair,
    cleanupSession,
    activeSockets,
